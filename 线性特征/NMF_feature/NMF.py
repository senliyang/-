...
A = pd.read_csv('MD_A.csv', header=0, index_col=0).values

feature_MFm, feature_MFd = get_low_feature(D, 0.01, pow(10, -4), A)

feature_MFm = pd.DataFrame(feature_MFm)
feature_MFm.to_csv('NMF_microbe_feature.csv',header=False,index=False)

feature_MF = pd.DataFrame(feature_MFd)
feature_MF.to_csv('NMF_disease_feature.csv',header=False,index=False)